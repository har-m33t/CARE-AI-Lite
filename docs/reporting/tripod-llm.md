...
| 6a | LLM name, version, last training date | Done | `carelite/config.py.Models`; digests pinned via `make pin-models`, recorded per generation (v3 §16). Training cutoff dates are Ollama model-card metadata, to be captured verbatim in the results appendix at run time. |
