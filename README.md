...
called the project further along than it was. As of 2026-08-25 the build is substantially
complete — see "Status" below for what remains.
...
├── DECISIONS.md                 # Dated record of project-owner decisions (D1-D12)
...
│
├── figures/                       # Regenerated by `make reproduce` (carelite-viz), PNG+PDF pairs
...
and the repository as of 2026-08-25, not a plan. See `docs/decisions/README.md` for the decisions
...
| Generation orchestration (six experimental conditions) | Complete for A, A2, B, C, D (180 cells each); **Condition LC stopped at 39/180 by decision** (`DECISIONS.md` D11 — cost ~33x the other conditions per cell on rented GPU hardware) |
| LLM-as-judge and its validation study | Judging complete: 939/939, zero errors, 206 minutes. Confirmatory status still blocked on human-rating data (unchanged); the n=30 validation study's instrument-check findings are confirmed at n=939 — see `docs/limitations.md` §4 for the `naturalness`/`ritualistic` measurement failure this surfaced |
...
| Full evaluation run | **Complete at 939 generations** (5 conditions × 180 + 39 partial LC cells, `DECISIONS.md` D11), on rented GPU hardware (see `docs/limitations.md` §4) — **every result is descriptive, per `DECISIONS.md` D10, not confirmatory or pre-specified** |
...
| Statistical analysis (Friedman/Wilcoxon/Holm-Bonferroni, mixed-effects) | In progress (`carelite-stats`); owns the results write-up — see that document for effect sizes and corrected tests, not here |
| Figures | In progress (`carelite/viz/`) — some figures render now (e.g. retrieval quality); most that depend on `rubric_score` await that table's load from `runs/judge-holdout/` |
...
| `make reproduce` | Built and wired to `carelite.viz.reproduce` (figures render where their source data exists); `carelite.stats` exists but not yet under the `run(output_dir)` contract this entry point looks for, so tables report `[pending]` |
